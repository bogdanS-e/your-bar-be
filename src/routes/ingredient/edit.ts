...
import { editCustomIngredientById, getCustomIngredientByEmailAndId } from '../../db/schemas/custom-ingredients';
...
const editIngredientRoute = (ingredientRouter: Router) => {
  ingredientRouter.put('/edit/:ingredientId', protectedRoute, multerUpload.single('image'), validateIngredient, async (req, res: Response<IEditIngredientRes | IResError>) => {
    const { name, description, tags } = req.body as IEditIngredientReq;
    const { ingredientId } = req.params;
    let imageUrl = null;

    const oldIngredient = await getCustomIngredientByEmailAndId(req.auth.payload.email as string, ingredientId);
